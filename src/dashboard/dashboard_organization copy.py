...
class OrganizationGitHubStats:
    def __init__(self):
        load_dotenv()
        self.repository = os.getenv("GITHUB_REPOSITORY")  # Single repository
        self.token = os.getenv("GITHUB_TOKEN")
        self.issues_df = pd.DataFrame()
        self.monte_carlo_simulations = 1000  # Number of Monte Carlo simulations to run
        self.output_dir = "organization_charts"
    ...
    def fetch_issues(self):
        ...
        try:
            ...
            print(f"✅ Carregadas {len(self.issues_df)} issues.")
        except Exception as e:
            print(f"❌ Erro ao buscar issues: {str(e)}")
    ...
    def run(self):
        ...
        print("🚀 Iniciando análise de issues da organização...")
        
        # Fetch all issues
        self.fetch_issues()
        
        if len(self.issues_df) == 0:
            print("❌ Nenhuma issue para analisar. Encerrando.")
            return
        ...
        completion_msg = "Organização já concluiu todas as issues!" if mc_results.get('completion_date_p50') == "Complete" else f"Data provável de conclusão: {mc_results.get('completion_date_p50', 'N/A')}"
        print(f"\n✅ Análise concluída! {completion_msg}")
